...
The test dataset consists of \textbf{8 users}, \textbf{3 workspaces}, \textbf{11 channels} (covering public, private, and direct types), and \textbf{over 120 messages} with realistic conversation content.
...
\begin{itemize}
    ...
    \item \textbf{120+ messages} are distributed across all 11 channels with natural conversation patterns covering topics such as curriculum planning, paper submissions, robotics competitions, hardware procurement, and database schema design.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{ChatWindow.js} --- message display with timestamps, message input with emoji picker
...
\begin{itemize}
    ...
    \item \textbf{Search:} Use the Search button in the top bar to find messages containing a keyword across all channels you have access to.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \texttt{login(username="carolwang", password="carol789")}
    ...
    \begin{itemize}
        \item \texttt{login(username="henrykim", password="henry234")}
        \item \texttt{get\_messages(chid=12)} $\Rightarrow$ Returns Carol's message.
    \end{itemize}
\end{enumerate}

\subsection{Session 5: Message Search with Access Control}

\begin{enumerate}
    \item Alice searches for ``PostgreSQL'':
    \begin{itemize}
        \item \texttt{search\_messages(keyword="PostgreSQL")}
        \item Results include messages from WS3 \#general (where she is a member) but \textbf{not} from WS2 channels (where her invite is still pending).
    \end{itemize}

    \item Dave searches for the same keyword:
    \begin{itemize}
        \item \texttt{search\_messages(keyword="PostgreSQL")}
        \item Dave gets results only from WS2 channels where he is an accepted member. He does not see WS3 results because his WS3 invite is still pending.
    \end{itemize}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Message Search with Access Control.} The search function respects workspace and channel membership --- users can only find messages in channels they have been accepted into. The search uses PostgreSQL's \texttt{ILIKE} for case-insensitive matching.
...
The test dataset includes 8 users across 3 workspaces with 11 channels and over 120 messages, providing rich data for demonstrating all features during the demo.
